...
void loop() {
   ...
   delay(500);
    ...
    y = map(y, 0, 1023, 0, 100); //se escala la señal, 0 es a 0 como 1023 es a 1000, arduino tiene convertidor A/D de 10 bits (pasa 0 a 5V a 0 a 1023)
    r = analogRead(referenciaPin); //lee la salida del sistema
    r = map(r, 0, 1023, 0, 100); //se escala la señal, 0 es a 0 como 1023 es a 1000, arduino tiene convertidor A/D de 10 bits (pasa 0 a 5V a 0 a 1023)
    ek =  r - y; //se calcula el error

    uk = uk_1 + Kp*((1+deltat/Ti+Td/deltat)*ek-(1+2*Td/deltat)*ek_1+(Td/deltat)*ek_2);

    if (uk > 1000) { //Bloque de saturacion de la salida del controlador
    uk_lim= 1000;
    }
    else if (uk < 0) {
    uk_lim = 0;
    ...
    uk_lim = uk_lim*255/1000; // Se escala de 0 a 255 por ser PWM con 8 bits
